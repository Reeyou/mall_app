...
export default class GoodsDetail extends Component {
  ...
  _getProductDetail() {
    getProductDetail({spu_id: 1}).then(res => {});
  }
  componentDidMount() {
    this.loadData();
  }
  loadData() {
    console.log(this.props.navigation.state.params.goods_id);
    getProductDetail({spu_id: this.state.goods_id}).then(res => {
      console.log(res.data.spu);
      this.setState({
        goods_info: res.data.spu[0],
      });
    });
  }
  getLeftButton() {
    return (
      <TouchableOpacity style={styles.navBtn}>
        <Ionicons name={'ios-arrow-back'} size={22} style={{color: '#000'}} />
      </TouchableOpacity>
    );
  }
  getRightButton() {
    return (
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        ...
        <TouchableOpacity style={styles.navBtn}>
          <Ionicons name={'ios-more'} size={20} style={{color: 'black'}} />
        </TouchableOpacity>
      </View>
    );
  }
      ...
      <View style={[styles.goods_info, styles.addPadding]}>
        ...
        <View style={styles.goods_info_header}>
          <Text style={styles.price}>
            ￥<Text style={styles.price_pic}>3599</Text>.00
          </Text>
        ...
        <View style={{flex: 1}}>
          <Text style={styles.goods_name}>
            {this.state.goods_info.product_name}
          </Text>
  ...
  renderGoodsPromotion() {
    return (
      ...
      <View style={[styles.promotion, styles.addPadding]}>
        ...
        <Ionicons name={'ios-more'} size={18} style={{color: '#666'}} />
      </View>
    );
  }
  renderGoodsSku() {
    return (
      <View style={styles.goods}>
        {/* sku */}
        <View style={[styles.goods_sku, styles.padding_h]}>
          <View style={styles.goods_sku_content}>
            <Text style={styles.tip}>已选</Text>
            <View style={[styles.goods_item, styles.border]}>
              <Text style={styles.goods_text}>
                丹霞橙(8+256G)，1件，可选服务
              </Text>
              <Ionicons name={'ios-more'} size={18} style={{color: '#666'}} />
            </View>
        ...
        <View style={[styles.goods_flex, styles.padding_h]}>
          <Text style={styles.tip}>送至</Text>
          <View style={[styles.goods_item, styles.border]}>
            <Text style={styles.goods_text}>广东广州市白云区城区</Text>
            <Ionicons name={'ios-more'} size={18} style={{color: '#666'}} />
          </View>
        </View>
        <View
          style={[styles.goods_flex, styles.goods_express, styles.padding_h]}>
          <Text style={styles.tip}>运费</Text>
          <View style={styles.goods_item}>
            <Text style={styles.goods_text}>商家免邮</Text>
          </View>
        </View>
      </View>
    );
      ...
      <View style={[styles.goods_comment, styles.addPadding]}>
        <View style={styles.goods_comment_header}>
          <View style={styles.header_left}>
            ...
            <Text style={styles.goods_comment_label}>
              商品评价(<Text style={styles.goods_comment_count}>12</Text>)
            </Text>
          </View>
          <View style={styles.header_Right}>
            <Text style={styles.goods_comment_percent}>
              好评度<Text style={styles.percent}>100%</Text>
            </Text>
            <AntDesign name={'right'} size={12} style={{color: '#666'}} />
          </View>
        </View>
        <View style={styles.goods_comment_container}>
          ...
          <View style={styles.goods_comment_content}>
            <Text style={styles.comment_text}>
              手机收到了，很漂亮，应该质量很好，看到很满意，特别是物流很快，下单以后一天就收到了，很满意，谢谢😊
            </Text>
    ...
    return (
      ...
      <View style={styles.goods_detail}>
        <View style={styles.detail_header}>
          <View style={styles.detail_header_line} />
    ...
    return (
      <View style={[styles.detail_cart, styles.addPadding]}>
        ...
        <View style={styles.cart_add}>
          <EvilIcons name={'cart'} size={26} style={{color: '#333'}} />
